export function createDAG(nodes) {
  ...
  for (const node of nodes) {
    ...
    // Normalize: ensure deps is always an array (default to empty array)
    map.set(node.id, { ...node, deps: node.deps || [] })
    ids.push(node.id)
  }
...
export function dependents(dag, id) {
  if (!dag.nodes.has(id)) return []
  return dag.ids.filter(nid => dag.nodes.get(nid).deps.includes(id))
}

export function isReady(dag, id, completed) {
  if (!dag.nodes.has(id)) return false
  if (completed.includes(id)) return false
  return dag.nodes.get(id).deps.every(dep => completed.includes(dep))
}
