...
export async function reset({ atomPath, targetState, newBranch, baseBranch, exec }) {
  if (newBranch) {
    validateBranch(newBranch)
  }

  // Checkout target state (commit SHA or branch)
  if (targetState) {
    try {
      await run("git", ["-C", atomPath, "checkout", targetState], exec)
    } catch (err) {
      // Checkout failed, but still clean
    }

    // If newBranch specified, create it from current position
    if (newBranch) {
      try {
        await run("git", ["-C", atomPath, "checkout", "-b", newBranch], exec)
      } catch (err) {
        // Branch creation failed, but still clean
      }
    }
  } else if (baseBranch) {
    try {
      await run("git", ["-C", atomPath, "checkout", baseBranch], exec)
    } catch (err) {
      // Checkout failed, but still clean
    }
  }

  // Always clean working directory
  await run("git", ["-C", atomPath, "clean", "-fdx"], exec)
}

export async function recycleAtom({ atomPath, exec, commitMessage = "recycle: atom cleanup" }) {
  // Stage all changes
  await run("git", ["-C", atomPath, "add", "-A"], exec)

  // Commit (allow empty)
  try {
    await run("git", ["-C", atomPath, "commit", "--allow-empty", "-m", commitMessage], exec)
  } catch (err) {
    // Commit failed, but continue with cleanup
  }

  // Detach HEAD
  await run("git", ["-C", atomPath, "checkout", "--detach"], exec)
