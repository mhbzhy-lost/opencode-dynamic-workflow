...
export class AtomPool {
  ...
  async merge(sourceAtom, targetAtom) {
    ...
    // 在 target atom 执行 merge（使用 execFileSync argv 数组，禁止 shell 拼接）
    try {
      execFileSync(
        "git",
        ["merge", sourceBranch, "--no-ff", "-m", `merge ${sourceBranch}`],
      ...
      if (stderr.includes("CONFLICT") || stdout.includes("CONFLICT")) {
        ...
        execFileSync("git", ["merge", "--abort"], { cwd: targetAtom.cwd, stdio: "ignore" })
        throw new Error(
  ...
  async _createAtom() {
    ...
    execFileSync("mkdir", ["-p", worktreeDir], { stdio: "ignore" })
