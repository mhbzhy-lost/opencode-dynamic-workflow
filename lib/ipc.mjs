...
export function readCommand(commandsDir, type, id) {
  ...
  try {
    ...
    // Tolerant reader: partial/in-flight writes from the main agent can
    // produce malformed JSON between existsSync() and readFileSync().
    // Return null so the caller retries on the next poll tick instead
    // of crashing the command loop with a cryptic JSON error.
    return null;
  }
}
...
export function createIpc(workdir) {
  ...
  // Defensive serialization: ensures advancePhase read-mutate-write is
  // sequenced even if fs ops become async in the future.
  let _statusWrite = Promise.resolve();
