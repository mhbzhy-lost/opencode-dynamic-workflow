...
export function resolveWorkflowConfig(args = [], userDefaults = {}) {
  ...
  const positional = []
  const VALUE_FLAGS = new Set(["--model", "--base-url", "--workdir", "--max-concurrent"])

  for (let i = 0; i < args.length; i++) {
    const a = args[i]
    if (VALUE_FLAGS.has(a) && i + 1 >= args.length) {
      throw new Error(`missing value for ${a}`)
    }
    switch (a) {
      case "--model":             result.model = args[++i]; break
      case "--base-url":          result.baseUrl = args[++i]; break
      case "--workdir":           result.workdir = args[++i]; break
      case "--max-concurrent":    result.maxConcurrent = Number(args[++i]); break
    ...
    shutdown: async () => {
      ...
      if (config.worktree?.autoMerge && worktreeApi?.mergeAccumulator && accumulatorDir) {
        await worktreeApi.mergeAccumulator(
          accumulatorDir,
          config.worktree.baseBranch || "main",
          { workflowId: config.id || `wf-${Date.now()}` }
        )
      }
