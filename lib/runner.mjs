...
// createOpencodeServer SDK does not yet accept a cwd option, so we use
// process.chdir() to point it at the worktree. Concurrent chdir-swap
// would race, so throw if a second workflow attempts it.
...
export async function createWorkflow(config = {}) {
  ...
  if (config.worktree?.enable && !config.baseUrl) {
    ...
    if (!config._mockClient) {
      ...
      // Fall through to IPC/state setup below using the just-started server.
    }
  }
    ...
    dag: async (nodeSpecs) => {
      ...
      for (let i = 0; i < layerList.length; i++) {
        ...
        const specs = layer.map(id => {
          ...
          // Use String.replaceAll (literal, Node 15+) instead of RegExp
          // to avoid ReDoS when dep IDs contain regex metacharacters
          // (e.g., `.`, `+`, `[`, `(`).
