...
export function createWorktreeApi({ repoDir, baseBranch, exec }) {
  ...
  async function mergeAccumulator(accumulatorDir, baseBranch) {
    const exec = wtExec
    const accBranch = `${baseBranch}-acc`
    await exec("git", ["-C", accumulatorDir, "add", "."])
    await exec("git", ["-C", accumulatorDir, "commit", "-m", "workflow: final accumulator state", "--allow-empty"])
    await exec("git", ["-C", wtRepoDir, "checkout", baseBranch])
    await exec("git", ["-C", wtRepoDir, "merge", "--no-ff", "-m", "workflow: merge accumulator into base", accBranch])
  }
